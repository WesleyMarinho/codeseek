...
function showFormModal({ title, initialData = {}, onSubmit, submitText, cancelText = 'Cancel', fields = [], customContent = '' }) {
    // Remove qualquer modal existente para evitar duplicatas
    ...
    const renderField = (field, value) => {
        if (field.type === 'textarea') {
            ...
            return `
                ...
                <textarea id="modal-${field.name}" name="${field.name}" class="block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-blue-500 focus:border-blue-500" rows="${rows}" ${field.required ? 'required' : ''}>${value}</textarea>`;
            ...
            return `
                <label for="modal-${field.name}" class="block text-sm font-medium text-gray-700 mb-1">${field.label}</label>
                <select id="modal-${field.name}" name="${field.name}" class="block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-blue-500 focus:border-blue-500" ${field.required ? 'required' : ''}>
                    ${optionsHTML}
                </select>`;
            ...
            return `
                <div class="flex items-center h-full">
                    <input type="checkbox" id="modal-${field.name}" name="${field.name}" ${checked} class="h-5 w-5 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
            ...
            return `
                <div class="col-span-full">
                    ...
                    <div id="${uniqueId}" class="dropzone border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-gray-400 transition-colors">
                        <div class="dz-message" data-dz-message>
                            <span class="text-gray-500">Arraste arquivos aqui ou use o botão acima</span>
                        </div>
                    </div>
                    <div id="media-preview-grid" class="media-preview-grid mt-4"></div>
                    <div class="mt-2">
            ...
            return `
                <label for="modal-${field.name}" class="block text-sm font-medium text-gray-700 mb-1">${field.label}</label>
                <input type="${field.type}" id="modal-${field.name}" name="${field.name}" value="${value}" ${stepAttr} class="block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-blue-500 focus:border-blue-500" ${field.required ? 'required' : ''}>`;
        }
    };
    ...
    // Animação de entrada e inicialização do Dropzone
    setTimeout(() => {
        modal.style.opacity = '1';
        modalContent.classList.remove('scale-95', 'opacity-0');
        if (firstInput) firstInput.focus();
        
        // Inicializar Dropzone se houver campo de arquivo
        const dropzoneField = fieldsToRender.find(f => f.type === 'file');
        if (dropzoneField) {
            initializeDropzone(modal, dropzoneField, initialData.media || initialData.Media || []);
        }
    }, 50);
    ...
    form.onsubmit = (e) => {
        ...
        // Adicionar arquivos NOVOS do Dropzone ao FormData
        if (dropzoneInstance) {
        ...
        // Adicionar o caminho da mídia de destaque ao FormData
        const featuredItem = modal.querySelector('.media-preview-featured');
        if (featuredItem) {
            formData.append('featuredMedia', featuredItem.dataset.path);
        }
...
// --- Função para inicializar Dropzone ---
function initializeDropzone(modal, fieldConfig, existingMedia = []) {
    const dropzoneEl = modal.querySelector('.dropzone');
    ...
    const myDropzone = new Dropzone(dropzoneEl, {
        ...
        maxFiles: 10,
        acceptedFiles: fieldConfig.accept || "image/*,video/*"
    });
    
    // Armazenar a instância no modal para acesso posterior
    modal.dropzoneInstance = myDropzone;
    ...
    myDropzone.on("success", (file, response) => {
        ...
        if (response.success) {
            // Renderizar preview usando dados da resposta
            renderMediaPreview(previewGrid, { 
                ...response, 
    ...
    myDropzone.on("error", (file, errorMessage) => {
        ...
        showNotification(`Erro no upload: ${errorMessage}`, 'error');
    ...
    // Renderizar mídias já existentes
    existingMedia.forEach(media => {
        renderMediaPreview(previewGrid, media, false);
    });
}

// --- Função para renderizar preview de mídia ---
function renderMediaPreview(grid, mediaData, isExisting = true) {
    const isFeatured = isExisting && mediaData.isFeatured;
    const previewId = isExisting ? `media-${mediaData.id}` : `file-${mediaData.uuid}`;
    const mediaUrl = mediaData.url || mediaData.path;
    
    const item = document.createElement('div');
    item.className = `media-preview-item ${isFeatured ? 'media-preview-featured' : ''}`;
    item.dataset.id = previewId;
    item.dataset.path = mediaUrl;
    item.dataset.mediaId = mediaData.id || '';
    item.dataset.fileId = mediaData.uuid || '';
    
    item.innerHTML = `
        <img src="${mediaUrl}" class="w-full h-full object-cover" alt="Preview">
        ${isFeatured ? '<div class="media-preview-featured-badge"><i class="fas fa-star mr-1"></i>Destacado</div>' : ''}
        <div class="media-preview-overlay">
            <div class="media-preview-info">
                <div class="text-xs font-medium truncate">${mediaData.name || 'Mídia'}</div>
                <div class="text-xs opacity-75">${mediaData.size ? `${(mediaData.size / 1024).toFixed(1)} KB` : ''}</div>
            </div>
            <div class="media-preview-actions">
                ${!isFeatured ? '<button type="button" class="set-featured-btn"><i class="fas fa-star mr-1"></i>Destacar</button>' : ''}
                <button type="button" class="remove-media-btn"><i class="fas fa-trash mr-1"></i>Remover</button>
            </div>
        </div>
    `;
    ...
    // Se é o primeiro item, destacar automaticamente
    if (grid.children.length === 1 && !isFeatured) {
        setFeaturedPreview(item);
    }
}

// --- Event Listeners delegados para gerenciamento de mídia ---
document.addEventListener('click', (e) => {
    ...
    if (e.target.closest('.remove-media-btn')) {
        ...
        const previewItem = btn.closest('.media-preview-item');
        removeMediaPreview(previewItem);
    }
});
...
function setFeaturedPreview(previewItem) {
    ...
    container.querySelectorAll('.media-preview-item').forEach(item => {
        item.classList.remove('media-preview-featured');
        const badge = item.querySelector('.media-preview-featured-badge');
    ...
    previewItem.classList.add('media-preview-featured');
...
function removeMediaPreview(previewItem) {
    if (!previewItem) return;
    
    const modal = previewItem.closest('#form-modal');
    const dropzoneInstance = modal?.dropzoneInstance;
    const fileId = previewItem.dataset.fileId;
    const mediaId = previewItem.dataset.mediaId;
    
    // Remover do Dropzone se for arquivo novo
    if (fileId && dropzoneInstance) {
        const file = dropzoneInstance.files.find(f => f.upload.uuid === fileId);
        if (file) {
            dropzoneInstance.removeFile(file);
        }
    }
    
    // Para mídia existente, marcar para exclusão (implementar conforme necessário)
    if (mediaId) {
        console.log('Marcar mídia existente para exclusão:', mediaId);
    }
    
    // Verificar se era o item destacado
    const wasFeatured = previewItem.classList.contains('media-preview-featured');
